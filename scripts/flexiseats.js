(function($) {
  $.fn.flexiSeats = function(options) {
    ...
    this.on("click", "input:checkbox", function() {
      if ($(this).data("status") == "booked") return false;
      ...
      if (settings.multiple === true) {
        ...
        if ($(this).prop("checked") == true) selectSeat(_id);
        else {
          deselectSeat(_id);
        }
      }
    });
    $("#btnDefineDelete").on("click", () => {
      deleteSeat();
    });
    //Private Functions
    ...
    function draw(container) {
      ...
      var _rowLabel = $(
        '<div class="row"><span class="row-label"></span></div>'
      );
    ...
    function selectMultiple(start, end) {
      ...
      for (i = parseInt(_i[0]); i <= parseInt(_j[0]); i++) {
        for (j = parseInt(_i[1]); j <= parseInt(_j[1]); j++) {
          if (
            $('input:checkbox[id="seat' + i + "-" + j + '"]', scope).data(
              "status"
            ) != "notavailable" &&
            $('input:checkbox[id="seat' + i + "-" + j + '"]', scope).data(
              "status"
            ...
            $('input:checkbox[id="seat' + i + "-" + j + '"]', scope).prop(
              "checked",
              "checked"
            );
            selectSeat(i + "-" + j);
